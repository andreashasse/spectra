...
encode(_, _CallerTypeInfo, {type, color, 0}, {R, G, B}, _TargetType, _Config) when
    is_integer(R),
    is_integer(G),
    is_integer(B),
    R >= 0,
...
encode(_, _CallerTypeInfo, {type, color, 0}, Data, _TargetType, _Config) ->
...
decode(_, _CallerTypeInfo, {type, color, 0}, <<"#", R1, R2, G1, G2, B1, B2>>, _TargetType, _Config) ->
    R = list_to_integer([R1, R2], 16),
    G = list_to_integer([G1, G2], 16),
    B = list_to_integer([B1, B2], 16),
    {ok, {R, G, B}};
decode(_, _CallerTypeInfo, {type, color, 0}, Data, _TargetType, _Config) ->
    {error, [sp_error:type_mismatch({type, color, 0}, Data)]}.
