...
simple_type_validation_test() ->
    IntSchema = spectra:schema(json_schema, ?MODULE, {type, user_id, 0}, [pre_encoded]),
...
string_type_validation_test() ->
    StringSchema = spectra:schema(json_schema, ?MODULE, {type, user_name, 0}, [pre_encoded]),
    %% Remove $schema field since jesse doesn't support 2020-12
...
binary_type_validation_test() ->
    BinarySchema = spectra:schema(json_schema, ?MODULE, {type, user_email, 0}, [pre_encoded]),
    %% Remove $schema field since jesse doesn't support 2020-12
...
record_validation_test() ->
    RecordSchema = spectra:schema(json_schema, ?MODULE, {record, user_profile}, [pre_encoded]),
    %% Remove $schema field since jesse doesn't support 2020-12
...
complex_map_validation_test() ->
    MapSchema = spectra:schema(json_schema, ?MODULE, {type, user_settings, 0}, [pre_encoded]),
    %% Remove $schema field since jesse doesn't support 2020-12
