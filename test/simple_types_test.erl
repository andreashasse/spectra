...
missing_test() ->
    ...
    %% arity
    {ok, MyArityType} = spectra_type_info:find_type(TypeInfo, my_arity, 0),
    ?assertEqual(
        #sp_range{
            type = integer,
            lower_bound = 0,
    ...
    %% byte
    {ok, MyByteType} = spectra_type_info:find_type(TypeInfo, my_byte, 0),
    ?assertEqual(
        #sp_range{
            type = integer,
            lower_bound = 0,
    ...
    %% char
    {ok, MyCharType} = spectra_type_info:find_type(TypeInfo, my_char, 0),
    ?assertEqual(
        #sp_range{
            type = integer,
            lower_bound = 0,
    ...
    %% mfa
    {ok, MyMfaType} = spectra_type_info:find_type(TypeInfo, my_mfa, 0),
    ?assertEqual(
        #sp_tuple{
            fields =
                [
    ...
    %% any
    {ok, MyTermType} = spectra_type_info:find_type(TypeInfo, my_term, 0),
    ...
    %% timeout
    {ok, MyTimeoutType} = spectra_type_info:find_type(TypeInfo, my_timeout, 0),
    ?assertEqual(
        #sp_union{
            types =
                [
    ...
    %% pid
    {ok, MyPidType} = spectra_type_info:find_type(TypeInfo, my_pid, 0),
    ...
    {ok, MyIodataType} = spectra_type_info:find_type(TypeInfo, my_iodata, 0),
    ...
    {ok, MyIolistType} = spectra_type_info:find_type(TypeInfo, my_iolist, 0),
    ...
    %% port
    {ok, MyPortType} = spectra_type_info:find_type(TypeInfo, my_port, 0),
    ...
    %% reference
    {ok, MyReferenceType} = spectra_type_info:find_type(TypeInfo, my_reference, 0),
    ...
    %% node
    {ok, MyNodeType} = spectra_type_info:find_type(TypeInfo, my_node, 0),
    ...
    %% identifier
    {ok, MyIdentifierType} = spectra_type_info:find_type(TypeInfo, my_identifier, 0),
    ?assertEqual(
        #sp_union{
            types =
                [
    ...
    %% literal
    {ok, MyLiteralType} = spectra_type_info:find_type(TypeInfo, my_literal, 0),
    ...
    %% list
    {ok, MyListType} = spectra_type_info:find_type(TypeInfo, my_list, 0),
    ...
    %% term
    {ok, MyTermType2} = spectra_type_info:find_type(TypeInfo, my_term, 0),
    ?assertEqual(#sp_simple_type{type = term}, MyTermType2),
    ?assertEqual({ok, 42}, spectra_json:to_json(?MODULE, {type, my_term, 0}, 42)),
    ?assertEqual({ok, 42}, spectra_json:from_json(?MODULE, {type, my_term, 0}, 42)),

    %% nonempty_list
    {ok, MyNonemptyListType} = spectra_type_info:find_type(TypeInfo, my_nonempty_list, 0),
    ...
    %% nil
    {ok, MyNilType} = spectra_type_info:find_type(TypeInfo, my_nil, 0),
    ...
    %% dynamic
    {ok, MyDynamicType} = spectra_type_info:find_type(TypeInfo, my_dynamic, 0),
    ...
    {ok, MyNonemptyBinaryType} =
        spectra_type_info:find_type(TypeInfo, my_nonempty_binary, 0),
    ...
    %% bitstring
    {ok, MyBitstringType} = spectra_type_info:find_type(TypeInfo, my_bitstring, 0),
    ...
    {ok, MyNonemptyBitstringType} =
        spectra_type_info:find_type(TypeInfo, my_nonempty_bitstring, 0),
    ...
    %% no_return
    {ok, MyNoReturnType} = spectra_type_info:find_type(TypeInfo, my_no_return, 0),
    ...
    %% none
    {ok, MyNoneType} = spectra_type_info:find_type(TypeInfo, my_none, 0),
